...
import { CatalogClient, CATALOG_FILTER_EXISTS } from '@backstage/catalog-client';  // Updated: Use CatalogClient directly
import { Entity } from '@backstage/catalog-model';
import { DiscoveryApi } from '@backstage/core-plugin-api';  // NEW: For CatalogClient
import { AuthService } from '@backstage/backend-plugin-api';  // NEW: For auth
...
export interface RouterOptions {
  config: Config;
  discovery: DiscoveryApi;  // NEW
  auth: AuthService;  // NEW
}

export class SonarCloudScoreboardService {
  private readonly baseUrl: string = 'https://sonarcloud.io';
  private readonly apiKey: string = '8a554ec0b14ea126914b7d532a830c780b4bb542'; // Consider moving to config or secrets for production
  private readonly authType: string = 'Bearer';

  constructor(private config: Config) {}  // Updated: Remove catalogApi

  // NEW: Compute scores from pre-fetched entities (moved from getEntityScores)
  ...
  private async getSonarCloudScore(
    ...
    // Use entity annotation for project key
  ...
  private calculateTotalScore(scores: {
    ...
    const total =
      ...
      scores.vulnerabilitiesScore +
      scores.codeSmellsScore +
      scores.duplicationsScore;
...
export async function createRouter(
  ...
  router.get('/entity-scores', async (_req, res) => {
    try {
      ...
      // NEW: Generate token for service-to-service auth (since endpoint is unauthenticated, use own service credentials)
      const { token } = await auth.getPluginRequestToken({
        onBehalfOf: await auth.getOwnServiceCredentials(),
        targetPluginId: 'catalog',
      });

      // NEW: Create authenticated CatalogClient
      ...
      // NEW: Fetch entities using the authenticated client
      const { items: entities } = await catalogClient.getEntities({
        filter: {
          kind: 'Component',
          'metadata.annotations.sonarcloud.io/project-key': CATALOG_FILTER_EXISTS,
          // Optional: Add more filters, e.g., 'spec.type': 'service' or 'metadata.tags': ['java']
        },
      });

      // NEW: Compute scores from fetched entities
